...
# Areas with no analogous ERA5-Land coverage (a south strip and parts of the
# east) where the underlying microclimate prediction surfaces are nonsensical.
# These predictions perpetuate downstream into the connectivity/current
# surfaces, so they must be excluded from the mapped target domain the same
# way the omniscape resistance/source prep scripts already exclude them.
NA_CLIPPER_PATH = DATA / "NA_clipper.gpkg"
...
def build_na_exclusion_mask(profile: dict) -> np.ndarray:
    """True where pixels fall inside NA_clipper.gpkg (no analogous ERA5-Land
    coverage). Mirrors the na_remove_mask logic in
    CONNECTIVITY/CIRCUIT_CONNECTIVITY/prep_omni_resistance.py, but via
    rasterize instead of geometry_mask(invert=True) - both give the same
    "inside the clip polygons" boolean array."""
    if not NA_CLIPPER_PATH.exists():
        print(f"[WARN] missing NA_clipper: {NA_CLIPPER_PATH}; no ERA5-coverage exclusion applied.")
        return np.zeros((profile["height"], profile["width"]), dtype=bool)

    na_clipper = gpd.read_file(NA_CLIPPER_PATH).to_crs(profile["crs"])
    return rasterize_polygons_to_mask(na_clipper, profile)
...
def prepare_map_panel_data() -> dict:
    ...
    na_mask = build_na_exclusion_mask(profile)

    target_domain = peruspiiri_mask & vegetation_domain & (~water_mask) & (~na_mask)
    ...
    print(f"  NA (no ERA5 coverage)  = {int(np.sum(peruspiiri_mask & na_mask))}")
...
def draw_map_panel(fig: plt.Figure, ax, cax, map_data: dict) -> None:
    ...
    cbar.ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=100, symbol=""))
