...
for font_path in TAHOMA_FONT_PATHS:
    if font_path.exists():
...
PANEL_LINE_AXES_RECT = [0.13, 0.18, 0.80, 0.75]
...
LINE_ZONE_YMIN = -0.150
LINE_ZONE_LABEL_Y = -0.078
LINE_XLABEL_PAD = 34
...
LINE_EXPORT_PNG = True
LINE_EXPORT_PDF = True
LINE_EXPORT_SVG = True
LINE_TRANSPARENT_BACKGROUND = True
LINE_DPI = 300

MAP_EXPORT_PNG = True
MAP_EXPORT_PDF = True
MAP_EXPORT_SVG = True
MAP_TRANSPARENT_BACKGROUND = True
MAP_DPI = 600
...
def draw_line_panel(ax, overall_df: pd.DataFrame, band_df: pd.DataFrame) -> None:
    ...
    if not band_df.empty:
        ...
        ax.text(
            ...
            ha="center",
            va="center",
            bbox=dict(boxstyle="round,pad=0.26", facecolor="white", edgecolor=COL_RED_FILL, linewidth=1.0, alpha=0.88),
            zorder=8,
        )
        ax.text(
            nwn_box_xy[0],
    ...
    ax.text(
        ...
        ha="center",
        va="center",
        bbox=dict(boxstyle="round,pad=0.42", facecolor="white", edgecolor=COL_BLUE_FILL, linewidth=1.6, alpha=0.95),
        zorder=9,
    )
    ...
    ax.text(
        ...
        ha="center",
        va="center",
        bbox=dict(boxstyle="round,pad=0.42", facecolor="white", edgecolor=COL_RED_FILL, linewidth=1.6, alpha=0.95),
        zorder=9,
    )
...
def draw_map_panel(fig: plt.Figure, ax, cax, map_data: dict) -> None:
    ...
    cbar.ax.tick_params(labelsize=FS_AXIS)
...
def add_map_title(fig: plt.Figure) -> None:
    ...
    fig.text(
        ...
        PANEL_TITLE_Y,
        "Relative heatwave connectivity loss (p90 baseline), ±1°C tolerance",
        ha="center",
        va="bottom",
