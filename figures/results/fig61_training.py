...
out_dir = Path(
    r"\\ad.helsinki.fi\home\t\terschan\Desktop\paper1\scripts\figures\results"
)
...
# Paul Tol high-contrast colour-blind-safe scheme
# Blue = temperature / left axis
# Ochre = histogram / right axis
# Red = validation metrics
COL_BLUE        = "#004488"
...
def style_axis(ax):
    ...
    ax.grid(axis="y", color=COL_GRID, linewidth=0.8, alpha=0.65)
...
def save_single_panel(fig, filename_stem):
    ...
    # Fixed manual layout.
    # This keeps every exported panel the same physical dimensions.
    fig.subplots_adjust(
        left=0.18,
        right=0.86,
        bottom=0.20,
...
required_hourly_cols = {"hour_local", "mean_temp", "sd_temp"}
required_raw_cols = {"uCC", "temp", "sensor_id"}

missing_hourly_cols = required_hourly_cols - set(hourly.columns)
missing_raw_cols = required_raw_cols - set(df.columns)

if missing_hourly_cols:
    raise ValueError(f"Missing columns in hourly file: {missing_hourly_cols}")
...
fig, axes = plt.subplots(
    2,
    2,
    figsize=(PANEL_WIDTH * 2, PANEL_HEIGHT * 2),
