...
Note that we did not mask predictions for this animation, but you should not use HELMI predictions inside non-vegetated areas (see Limitations).
...
Apart from the aforementioned validation, HELMI's performance outside of the represented feature combinations has not been extensively validated and should be interpreted with caution. Generally, we do not expect HELMI to be accurate in:
