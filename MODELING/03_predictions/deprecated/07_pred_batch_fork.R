...
# ---- ERA5
# open ERA5 netcdf and extract everything, then close it
...
full_stack <- crop(full_stack, era_proj)  # crop to ERA5 coverage to remove southern fringe

log_mem("Applied prediction mask")
gc()

# ---- Write full_stack to disk so parallel workers can re-open it safely ----
# terra SpatRaster external pointers are not valid across forked processes;
...
# Workers inherit model and full_stack_path from parent via fork — no serialization needed.
# terra SpatRaster must still be re-opened from disk inside the worker (pointer not valid
# across fork boundary), but the ranger model is inherited directly in memory.
predict_tile <- function(tile) {
  library(terra)
  stack <- rast(full_stack_path)  # re-open from disk — pointer not fork-safe
  tile_ext <- ext(
  ...
  predict(full_tile, model,  # model inherited from parent process via fork
    filename  = tile$path,
    overwrite = TRUE,
...
# makeForkCluster: workers share parent memory via copy-on-write fork (Linux only).
# No socket serialization — avoids SIGPIPE errors when exporting large objects.
