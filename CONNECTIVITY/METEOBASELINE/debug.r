...
# to make debugging batch jobs easier
log_step <- function(msg) {
  cat(sprintf("[%s] %s\n", format(Sys.time(), "%H:%M:%S"), msg))
  flush.console()
}

# limits multithreading across libraries in the parent process
# its just a saveguard to prevent things getting spicy since idk how the puhti r apptainer behaves
...
# retrieves array job index and job id from the 
# index file to drive time selection and dir naming
array_id <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID"))
job_id   <- Sys.getenv("SLURM_JOB_ID")

log_step(sprintf("Array task %d started (job %s)", array_id, job_id))

# random startup jitter between array jobs
# to avoid puhti contention when many jobs start simultaneously
# just one of many i/o concurrency fixes i tried, but it doesnt harm
...
#########################################################
# SINE COSINE ENCODED TIME
#########################################################
# generates cyclical time features (hour and doy) in sine cosine encoding
# these are required dynamic predictors helmi uses, so they need to be present
template <- static_stack[[1]] # base of static variables
...
#########################################################
# combines static and dyanmic preds into one raster stack
# helmi requires all predictors in exact training order
# so we need to ensure the feature order is according to the deterministic feature order file 
full_stack <- c(
  static_stack,
  era_proj,
  r_hour_sin,
...
predict_tile <- function(tile){
    # retry file write couple times to stop i/o fuckery
  for(i in 1:3){ 
  ...
  stop("Tile failed after retries")
}
...
r <- retry_rast(stack_path)

local_worker_stack <- file.path(tmps[id], "stack_local.tif")

pred_stack <<- writeRaster(
  r,
  filename = local_worker_stack,
  overwrite = TRUE,
  gdal = c("COMPRESS=NONE")
)
